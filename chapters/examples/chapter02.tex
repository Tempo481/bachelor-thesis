...
Zweck, dass Daten in ausschließlich ANSI-Zeichen kodiert übertragen werden. Irreguläre Zeichen wie Umlaute werden nicht übertragen, um Probleme vorzubeugen \citep{base64:2006}. 

\section{OpenID Connect}
\label{sec:OpenIDConnect}
OpenID Connect ist eine Authentifizierungsschicht, die auf der OAuth2 Spezifikation
aufbaut. OpenID Connect baut darauf eine Authentifizierung auf, indem es in dem JSON Web Token
...
End-Nutzer authentifizieren und gegeben falls den Client autorisieren, damit der Client einen Authorization Code erhält \citep{oauth2:2012}.
...
Der Zugriff auf den Pfad /pets kann nicht gegeben werden, da in \autoref{lst:Input-DatenfürOpenPolicyAgent} der owner nicht dem user entspricht. bob@hooli.com ist ungleich zu alice@hooli.com. 
...
und Filter. TODO Servlet und Filter
...
Man kann mittels Spring Security einen OAuth2 Ressource Server implementieren, der 
...
Performance zu testen. Es kann Last auf den Server erzeugen und die Latenzen der Antworten des Servers sowie andere Metriken messen und protokollieren und diese Werte in Graphen und anderer Weise ausgeben \citep{jmeter:2021}.
...
Wenn T beispielsweise 1,2 Sekunden beträgt und die Tolerierbarkeitsgrenze 4T beträgt, ergeben sich folgende Werte:
...
Hybrid Flow abzuhandeln, um dadurch Token von einem Authorization Server zu erhalten. Hierbei ist Postman der Client der OAuth2 Spezifikation.
